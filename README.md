...
- **查额度太麻烦**：每 15 秒自动读取本机最新额度，不用打开 Codex、输入 `/status`。
- **重置后显示旧数据**：识别额度周期的重置时间，到点先恢复 100%，新快照出现后再以真实值为准。
...
- `~/.codex/sessions` 仅在本机读取，不修改、不上传。
- App 不读取或记录 Codex token、账号信息、对话内容或其他 session 内容。
- 网络访问仅用于检查 GitHub 公开 Release，以及只读获取 `willcodexquotareset.com` 的 reset 预告；不会发送 GitHub token、Codex token 或设备信息。
