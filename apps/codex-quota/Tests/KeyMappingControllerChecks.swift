...
struct KeyMappingControllerChecks {
    static func main() {
        ...
        let capture = KeyMappingShortcut(keyCode: 0xFFFC, flags: 0)
        check("system action is a valid target but never a physical trigger", capture.isValidTarget && !KeyMappingTrigger.shortcut(capture).isValid)
        let captureRule = KeyMappingRule(trigger: .shortcut(.init(keyCode: 110, flags: 0)), target: capture)
        DoubleCommandTapController.saveRules([captureRule], to: defaults); controller.reloadGesture(); posted.removeAll()
        _ = controller.handle(type: .keyDown, event: event(110, .keyDown))
        let repeatCapture = event(110, .keyDown); repeatCapture.setIntegerValueField(.keyboardEventAutorepeat, value: 1)
        _ = controller.handle(type: .keyDown, event: repeatCapture)
        _ = controller.handle(type: .keyUp, event: event(110, .keyUp)); controller.stop()
        check("selection screenshot emits exactly one Control Command Shift 4 pair", posted.count == 2 && posted.map(\.type) == [.keyDown, .keyUp] && posted.allSatisfy { $0.getIntegerValueField(.keyboardEventKeycode) == 21 && $0.flags == [.maskControl, .maskCommand, .maskShift] })
